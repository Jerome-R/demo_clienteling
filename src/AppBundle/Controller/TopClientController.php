...
class TopClientController extends Controller
{
    ...
    public function topClientSortantAction(Request $request)
    {
        ...
        //Set SQL query for export
        $sql = "SELECT tc.nom, tc.prenom, tc.email,tc.libelle_boutique_rattachement_topclient,tc.telephone1,tc.telephone2,tc.portable1,tc.portable2,tc.adresse1,tc.adresse2,tc.code_postal,tc.ville,tc.pays,tc.segment,tc.ca_3_ans,tc.ca_12_mois,tc.ca_histo,tc.prix_max_article_histo,tc.frequence_3_ans,tc.frequence_12_mois,tc.panier_moyen_histo,tc.date_1erachat,tc.date_dernier_achat FROM app_client_sortant AS cs
                LEFT JOIN app_client AS tc ON cs.client_id = tc.id ";
        ...
        if( $user->getRole() == 'ROLE_SIEGE' OR $user->getRole() == 'ROLE_ADMIN') {
            if ( $request->getMethod() == 'POST' ) {
                if( $session->get('filtre_trigger_client_boutique_tc') != '' ){
                    $libelleBoutiqueRattachement =  $session->get('filtre_trigger_client_boutique_tc');
                    $findTopClientsSortants = $em->getRepository('AppBundle:ClientSortant')->getClientsSortantsOfTheStore($options);
                    $csv_name = "archive_".$libelleBoutiqueRattachement;
                    $sql .= "WHERE tc.is_topclient_sortant = 1 AND tc.libelle_boutique_rattachement_topclient = '$libelleBoutiqueRattachement' ";
                }
                elseif($session->get('filtre_trigger_client_RM_tc') != '' ){
                    $rm = $session->get('filtre_trigger_client_RM_tc');
                    $findTopClientsSortants = $em->getRepository('AppBundle:ClientSortant')->getClientsSortantsOfTheRm($options);
                    $csv_name =  "archive_".$rm;
                    $sql .= "LEFT JOIN fos_user_user AS u ON u.libelle = tc.libelle_boutique_rattachement_topclient WHERE u.retail_manager = '$rm' AND tc.is_topclient_sortant = 1 ";
                }
                else{  
                    $findTopClientsSortants = $em->getRepository('AppBundle:ClientSortant')->getAllClientsSortants($options);
                    $sql .= "";
                    $csv_name = "archive";
                }
            }
        ...
        elseif( $user->getRole() == 'ROLE_RETAIL_MANAGER' ) {
            if ( $request->getMethod() == 'POST' ) {
                    ...
                    if( $session->get('filtre_trigger_client_boutique_tc') != '' ){
                        ...
                        $findTopClientsSortants = $em->getRepository('AppBundle:ClientSortant')->getClientsSortantsOfTheRm($options);
                        $csv_name =  "archive_".$user->getLibelle();
                        $sql .= "LEFT JOIN fos_user_user AS u ON u.libelle = tc.libelle_boutique_rattachement_topclient WHERE u.retail_manager = '$rm' AND tc.is_topclient_sortant = 1 ";
                    }
            }
            else{
        ...
        elseif( $user->getRole() == 'ROLE_DIRECTEUR' ) {
            ...
            if ( $request->getMethod() == 'POST' ) {
                if( $session->get('filtre_trigger_client_boutique_tc') != '' ){
                    ...
                    $findTopClientsSortants = $em->getRepository('AppBundle:ClientSortant')->getClientsSortantsOfTheStore($options);
                    $csv_name = "archive_".$user->getLibelle();
                    $sql .= "AND tc.libelle_boutique_rattachement_topclient = '$libelleBoutiqueRattachement' ";
                }
                else{
                    $findTopClientsSortants = $em->getRepository('AppBundle:ClientSortant')->getClientsSortantsOfTheDirector($options);
                    $csv_name = "archive_".$user->getLibelle();
                    $sql .= "LEFT JOIN fos_user_user AS u ON u.libelle = tc.libelle_boutique_rattachement_topclient WHERE u.directeur = '$dr' AND tc.is_topclient_sortant = 1 ";
                }
            }
            else{
                if( $session->get('filtre_trigger_client_boutique_tc') != '' ){
        ...
        if (!$form2->isSubmitted()) {
            //On créer la variable uniquement si on a pas submit le form2
            $session->set('topclient_sortant_sql', $sql);
        }

        //Paginator : Pagination
        $topClientsortants  = $this
                ->get('knp_paginator')
                ->paginate(
                    $findTopClientsSortants,
                    //$request->query->get('page', 1),//page number,
                    $page,
                    50//limit per page
        );
        ...
        if ($form2->isSubmitted()) {
            ...
            //Préparation et execution de la requête
            //var_dump($sql);
            $stmt = $pdo->prepare($session->get('topclient_sortant_sql'));
            $stmt->execute();
            ...
            $export->setUser($user->getLibelle());
            $export->setCreatedAt(new \DateTime());
            $export->setRequest($session->get('topclient_sortant_sql'));

            $em->persist($export);
            $em->flush();

            //suppression de la variable de session
            //$session->remove('topclient_sortant_sql');
        ...
        return $this->render('AppBundle:TopClient:topclientsortant.html.twig', array(
            'topClientsortants'     => $topClientsortants,
            'form'                  => $form->createView(),
            'form2'                 => $form2->createView(),
            'session'               => $_SESSION
            )
